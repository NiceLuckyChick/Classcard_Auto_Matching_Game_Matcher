...
* **🔄 실시간 상태 변경 감지** : 게임이 리셋되거나 새로운 판이 시작되는 순간(`wrapper-learn` 클래스 변화)을 실시간으로 감지하여 데이터를 자동으로 최신화합니다.
...
### 2. 스크립트 실행 및 조작 가이드
> ⚠️ **중요 (필독):** 본 매크로는 로그인까지만 자동으로 수행하며, 학습할 단어장 선택 및 게임 시작은 사용자의 수동 조작이 필요합니다.

* **단계 1. 인증 정보 입력**
  스크립트를 실행한 뒤 터미널 창에 아이디와 비밀번호를 입력합니다.
* **단계 2. 단어 세트 이동**
  크롬 브라우저가 자동으로 켜지면, 학습하고자 하는 **단어 세트 메인 페이지**로 직접 이동합니다. (URL에 `/set/`이 포함되면 자동으로 데이터 수집이 시작됩니다.)
* **단계 3. 매칭 게임 시작**
  수집 완료 메시지(`Data Saved.`)가 출력되면, **매칭 게임**을 클릭해 시작합니다.
* **단계 4. 자동 재동기화**
